...
describe('TvshowService', () => {
  ...
  const tvShowList: TVShowList = { results: [tvShowObject, tvShowObject, tvShowObject], total_pages: 1 };

  let serviceStub: any;

  beforeEach(() => {
    serviceStub = {
      getTVShows: () => of(tvShowList),
      getTVShowById: () => of(tvShowObject),
    };

    TestBed.configureTestingModule({
      imports: [HttpClientTestingModule],
      providers: [{ provide: TvshowService, useValue: serviceStub }],
    });
  ...
  it('should return tvshows', () => {
    service.getTVShows().subscribe((tvshows) => {
      expect(tvshows).toEqual(tvShowList);
    });
  });

  it('should return tvshow with the same id', () => {
    service.getTVShowById('1399').subscribe((tvshow) => {
      expect(tvshow.id).toBe(tvShowObject.id);
    });
  });
});
